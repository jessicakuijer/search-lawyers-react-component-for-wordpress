...
*/
add_action( 'init', 'lawyers_rewrite' );

function lawyers_rewrite() {
    global $wp;
    global $wp_rewrite;
    $wp->add_query_var('city');
    $wp->add_query_var('lawyerfullname');
    $wp->add_query_var('lawyerid');
    $rule = '^avocats/(\d+)/(\d+)/(\d+)/?$';
    $profil_page_id = '1896';
    $rewrite = 'index.php?page_id=' . $profil_page_id . '&city=$matches[1]' . '&lawyerfullname=$matches[2]' . '&lawyerid=$matches[3]'; //job/?id=
    // $rewrite = '/avocats/?city=' . $matches[1]; //job/?id=
